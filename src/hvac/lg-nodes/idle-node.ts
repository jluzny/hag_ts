...
async function validateSystemConfiguration(
  ...
  if (
    state.indoorTemp !== undefined &&
    (state.indoorTemp < -50 || state.indoorTemp > 60)
  ) {
...
function assessSystemHealth(
  ...
  if (
    state.lastError &&
    (new Date().getTime() - state.lastError.timestamp.getTime()) < 300000
  ) {
  ...
  const errorCount =
    recentHistory.filter((evaluation) =>
      evaluation.decision.includes('error') ||
      evaluation.reasoning.includes('error')
    ).length;

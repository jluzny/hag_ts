...
import { HVACDecisionContext, DecisionResult, AIEnhancedState } from '../../ai/types/ai-types.ts';
...
export async function aiEvaluationNode(
  ...
  try {
    ...
    if (aiEngine) {
      ...
      log.warning('⚠️ [AI Evaluation] No AI engine available, using fallback logic');
    }
 ...
 */
function applyDecision(state: HVACLangGraphState, decision: DecisionResult): HVACLangGraphState {
  const now = new Date();
...
function enhanceStateWithAI(
  ...
  aiEnhanced.aiMetrics.avgDecisionTime = 
    totalDecisionTime / aiEnhanced.aiContext.decisionHistory.length;
...
function makeFallbackDecision(state: HVACLangGraphState): DecisionResult {
  ...
  // Temperature-based logic
    ...
    if (tempDiff < -1.5 && systemMode !== SystemMode.COOL_ONLY) {
      action = 'heating';
      reasoning += `Indoor ${indoorTemp}°C is ${Math.abs(tempDiff).toFixed(1)}°C below target ${targetTemp}°C.`;
    } else if (tempDiff > 1.5 && systemMode !== SystemMode.HEAT_ONLY) {
      action = 'cooling';
      reasoning += `Indoor ${indoorTemp}°C is ${tempDiff.toFixed(1)}°C above target ${targetTemp}°C.`;
    } else {
      action = 'idle';
      reasoning += `Indoor ${indoorTemp}°C is within range of target ${targetTemp}°C.`;
    }
  }

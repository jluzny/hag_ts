 ...
 */
export async function heatingNode(state: HVACLangGraphState): Promise<HVACLangGraphState> {
    ...
    return {
      ...
      evaluationHistory: [
        ...state.evaluationHistory.slice(-19),
        {
          ...
          reasoning: `Heating activated: target ${getTargetHeatingTemp(state)}°C`,
          conditions: {
            indoorTemp: state.indoorTemp,
            outdoorTemp: state.outdoorTemp,
 ...
 */
function transitionToIdle(state: HVACLangGraphState, reason: string): HVACLangGraphState {
  return {
    ...state,
    currentMode: "idle",
    evaluationHistory: [

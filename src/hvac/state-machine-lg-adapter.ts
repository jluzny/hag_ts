...
export class LangGraphHVACStateMachineAdapter implements IHVACStateMachine {
  ...
  async updateTemperatures(indoor: number, outdoor: number): Promise<void> {
    this.logger.debug('🌡️ [LangGraph Adapter] Updating temperatures (compatibility)', {
      indoor,
      outdoor
    });
  ...
  async evaluateConditions(): Promise<void> {
    this.logger.debug('🎯 [LangGraph Adapter] Triggering evaluation (compatibility)');
  ...
  async send(event: any): Promise<void> {
    this.logger.debug('📤 [LangGraph Adapter] Sending event (compatibility)', { event });

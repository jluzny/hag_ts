...
export class SmartScheduler {
  ...
  async generateSchedule(
    horizonHours: number = this.config.defaultLookaheadHours,
  ): Promise<ScheduleItem[]> {
    this.logger.debug('📅 [Smart Scheduler] Generating schedule', {
      horizonHours,
      activeRules: Array.from(this.scheduleRules.values()).filter((r) =>
        r.enabled
      ).length,
    });
    ...
    try {
      ...
      if (this.learning) {
        ...
        const recommendations = await this.learning
      ...
      for (let hour = 0; hour < horizonHours; hour++) {
        ...
        if (this.analytics) {
          try {
            const tempPrediction = await this.analytics
              .predictIndoorTemperature(hour);
            weatherContext = {
      ...
      if (this.optimizer && schedule.length > 0) {
        schedule.forEach(async (item, index) => {
          try {
            const context = await this.createOptimizationContext(
              item,
              schedule[index - 1],
            );
      ...
      this.logger.error(
        '❌ [Smart Scheduler] Schedule generation failed',
        error,
      );
  ...
  async triggerAutomation(
    ...
    try {
      ...
      if (applicableRules.length === 0) {
        this.logger.debug(
          'ℹ️ [Smart Scheduler] No applicable rules for automation trigger',
        );
        return null;
      }
      ...
      this.logger.error(
        '❌ [Smart Scheduler] Automation trigger failed',
        error,
      );
      return null;
    }
  ...
  getScheduleStatus(): {
    ...
    const enabledRules = Array.from(this.scheduleRules.values()).filter((r) =>
      r.enabled
    );
    const now = new Date();
  ...
  private async processScheduledEvents(): Promise<void> {
    ...
    for (const item of dueItems) {
      try {
        ...
        this.logger.error(
          '❌ [Smart Scheduler] Failed to execute schedule item',
          error,
        );
      }
    }
  ...
  private async updateWeatherBasedAutomation(): Promise<void> {
    ...
    try {
      ...
      const tempChange = Math.abs(
        tempPrediction.predictedValue - (context.indoorTemp || 21),
      );
      ...
      if (tempChange > 2 && tempPrediction.confidence > 0.6) {
        await this.triggerAutomation(
          'weather_change',
          context,
          `Significant temperature change predicted: ${
            tempPrediction.predictedValue.toFixed(1)
          }°C`,
        );
      }
    } catch (error) {
  ...
  private ruleApplies(
    ...
    if (triggers.seasonalConditions && context) {
      ...
      if (outdoorTemp !== undefined) {
        if (
          conditions.minOutdoorTemp !== undefined &&
          outdoorTemp < conditions.minOutdoorTemp
        ) return false;
        if (
          conditions.maxOutdoorTemp !== undefined &&
          outdoorTemp > conditions.maxOutdoorTemp
        ) return false;
      }
    }
  ...
  private async createScheduleItem(
    ...
    if (applicableRules.length > 0) {
      ...
      priority = primaryRule.priority >= 8
        ? 'high'
        : primaryRule.priority >= 5
        ? 'medium'
        : 'low';
    }
    ...
    if (personalizedWeights && this.learning) {
      const personalizedRecs = await this.learning
  ...
  private calculateAverageTargetTemp(schedule: ScheduleItem[]): number {
    ...
    return schedule.reduce((sum, item) => sum + item.targetTemp, 0) /
      schedule.length;
  }

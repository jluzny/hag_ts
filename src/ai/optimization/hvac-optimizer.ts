...
import { HVACDecisionContext, EnergyOptimizationResult, ScheduleItem } from '../types/ai-types.ts';
...
export class HVACOptimizer {
   ...
   */
  async optimizeDecision(context: HVACDecisionContext): Promise<OptimizationDecision> {
  ...
  async optimizeSchedule(
    ...
    try {
      ...
      for (let hour = 0; hour < horizonHours; hour++) {
        const timeSlot = new Date(currentTime.getTime() + hour * 60 * 60 * 1000);
      ...
      const projectedSavings = this.calculateProjectedSavings(schedule, context);
      
      return {
        recommendedSchedule: schedule,
        projectedSavings,
        tradeoffs: {
          comfortImpact: projectedSavings.percentage > 15 ? 'moderate' : 'minimal',
      ...
      this.logger.error('❌ [HVAC Optimizer] Schedule optimization failed', error);
      throw error;
    }
  }
  ...
  private generatePossibleActions(context: HVACDecisionContext): string[] {
    ...
    if (context.systemMode !== SystemMode.COOL_ONLY && 
        context.systemMode !== SystemMode.OFF) {
      actions.push('heating');
    }
    
    // Add cooling if allowed
    if (context.systemMode !== SystemMode.HEAT_ONLY && 
        context.systemMode !== SystemMode.OFF) {
      actions.push('cooling');
    }
    
    // Add off if system allows
    if (context.systemMode === SystemMode.OFF || 
  ...
  private async evaluateAction(
    ...
    // Calculate scores
    const comfortScore = this.calculateComfortScore(predictions.resultTemp, context);
    const energyScore = this.calculateEnergyScore(predictions.energyUse, action);
    const costScore = this.calculateCostScore(predictions.cost, context);
    
    // Calculate weighted overall score
    const overallScore = 
      this.config.comfortWeight * comfortScore +
   ...
   */
  private calculateComfortScore(predictedTemp: number, context: HVACDecisionContext): number {
   ...
   */
  private calculateCostScore(cost: number, context: HVACDecisionContext): number {
   ...
   */
  private selectOptimalAction(actions: OptimizationDecision[]): OptimizationDecision {
  ...
  private generateReasoning(
    ...
    reasoning += `Predicted temperature: ${predictions.resultTemp.toFixed(1)}°C. `;
    reasoning += `Energy use: ${predictions.energyUse.toFixed(2)} kWh. `;
    reasoning += `Cost: $${predictions.cost.toFixed(3)}. `;
    
    reasoning += `Scores - Comfort: ${(scores.comfortScore * 100).toFixed(0)}%, `;
  ...
  private getEnergyPriceForHour(hour: number): { level: string; rate: number } {
    ...
    return {
      ...
      rate: isPeakHour ? this.config.energyRates.peak : this.config.energyRates.offPeak
    };
  }
  ...
  private calculateProjectedSavings(
    ...
    const optimizedEnergy = schedule.reduce((total, item) => 
      total + (item.energyBudget || 1.5), 0
    );
  ...
  private fallbackDecision(context: HVACDecisionContext): OptimizationDecision {
    ...
    return {
      ...
      reasoning: 'Fallback optimization based on simple temperature differential.',
  ...
  private validateConfig(config: OptimizationConfig): OptimizationConfig {
    const totalWeight = config.comfortWeight + config.energyWeight + config.costWeight;
    
    if (Math.abs(totalWeight - 1.0) > 0.01) {
      throw new Error(`Optimization weights must sum to 1.0, got ${totalWeight}`);
    }

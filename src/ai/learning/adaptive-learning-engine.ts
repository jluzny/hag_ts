...
export interface BehavioralInsight {
  type: 'temperature_preference' | 'timing_pattern' | 'energy_sensitivity' | 'comfort_priority';
  description: string;
  confidence: number;
  evidence: string[];
  actionable: boolean;
...
export class AdaptiveLearningEngine {
  ...
  async getPersonalizedRecommendations(context: HVACDecisionContext): Promise<{
    ...
    this.logger.debug('🎯 [Learning Engine] Generating personalized recommendations', {
    ...
    try {
      ...
      const personalizedWeights = this.generatePersonalizedWeights(weightedPreference);
      
      const reasoning = this.generateRecommendationReasoning(matchingPreferences, weightedPreference);
      
      this.logger.info('✅ [Learning Engine] Personalized recommendations generated', {
      ...
      this.logger.error('❌ [Learning Engine] Failed to generate recommendations', error);
      return this.getDefaultRecommendations(context);
    }
  }
  ...
  async generateBehavioralInsights(): Promise<BehavioralInsight[]> {
    const now = new Date();
    const timeSinceLastAnalysis = now.getTime() - this.lastAnalysisTime.getTime();
    
    // Only regenerate insights every hour to avoid excessive computation
    if (timeSinceLastAnalysis < 60 * 60 * 1000 && this.cachedInsights.length > 0) {
    ...
    try {
      ...
      this.logger.error('❌ [Learning Engine] Failed to generate insights', error);
      return [];
    }
  }
  ...
  private async updateLearnedPatterns(): Promise<void> {
    ...
    const groupedInteractions = this.groupInteractionsByContext(recentInteractions);
  ...
  private async updatePreferenceWeights(): Promise<void> {
    ...
    recentFeedback.forEach(feedback => {
      ...
      const energy = 1 - Math.min(1, (feedback.actualOutcome.energyUsed || 2) / 4); // Normalize energy use
    ...
    if (totalScore > 0) {
      ...
      const weightChange = this.config.learningRate * this.config.maxWeightChange;
      
      this.currentWeights = {
        comfort: this.clampWeightChange(this.currentWeights.comfort, newComfortWeight, weightChange),
        energyEfficiency: this.clampWeightChange(this.currentWeights.energyEfficiency, newEnergyWeight, weightChange),
        cost: this.clampWeightChange(this.currentWeights.cost, newCostWeight, weightChange),
  ...
  private learnFromFeedback(feedback: LearningFeedback): void {
    ...
    if (outcome.userSatisfaction && outcome.userSatisfaction < 0.6) {
      ...
      if (matchingPrefs.length > 0) {
        ...
        this.logger.debug('📉 [Learning Engine] Reduced confidence due to low satisfaction', {
    ...
    if (outcome.energyUsed && outcome.energyUsed > 3.0) {
      ...
      this.currentWeights.energyEfficiency = Math.min(1.0, this.currentWeights.energyEfficiency + adjustment);
   ...
   */
  private findMatchingPreferences(context: HVACDecisionContext): LearnedPreference[] {
    return this.learnedPreferences.filter(pref => {
      const contextMatch = this.calculateContextSimilarity(pref.context, context);
  ...
  private calculateContextSimilarity(
    ...
    if (learnedContext.timeOfDay !== undefined && currentContext.currentHour !== undefined) {
      const hourDiff = Math.abs(learnedContext.timeOfDay - currentContext.currentHour);
    ...
    if (learnedContext.dayOfWeek !== undefined && currentContext.isWeekday !== undefined) {
      const isWeekday = currentContext.isWeekday;
      const learnedIsWeekday = learnedContext.dayOfWeek >= 1 && learnedContext.dayOfWeek <= 5;
      similarities.push(isWeekday === learnedIsWeekday ? 1.0 : 0.3);
    }
    
    // Temperature range similarity
    if (learnedContext.indoorTempRange && currentContext.indoorTemp !== undefined) {
   ...
   */
  private combinePreferences(preferences: LearnedPreference[]): LearnedPreference {
   ...
   * Generate personalized optimization weights
   */
  private generatePersonalizedWeights(preference: LearnedPreference): PreferenceWeights {
    const base = this.currentWeights;
    const personalizedAdjustment = 0.3; // How much to adjust from base weights
    
    return {
      comfort: base.comfort + (preference.preference.comfortPriority - 0.5) * personalizedAdjustment,
      energyEfficiency: base.energyEfficiency + (preference.preference.energyPriority - 0.5) * personalizedAdjustment,
  ...
  private analyzeTemperaturePreferences(): BehavioralInsight | null {
    ...
    const temperatures = tempOverrides.map(i => i.details.temperature as number);
    const avgTemp = temperatures.reduce((sum, temp) => sum + temp, 0) / temperatures.length;
    const tempRange = Math.max(...temperatures) - Math.min(...temperatures);
    
    return {
      type: 'temperature_preference',
      description: `User prefers temperature around ${avgTemp.toFixed(1)}°C with ${tempRange.toFixed(1)}°C variation`,
      confidence: Math.min(0.9, tempOverrides.length / 10),
      evidence: [`${tempOverrides.length} temperature overrides`, `Average: ${avgTemp.toFixed(1)}°C`],
      actionable: true,
      recommendation: `Adjust default target temperature to ${avgTemp.toFixed(1)}°C`
    };
  }
  ...
  private analyzeTimingPatterns(): BehavioralInsight | null {
    ...
    return {
      ...
      confidence: 0.7,
      evidence: [`${timedInteractions.length} timed interactions`, `Peak: ${peakHour}:00`],
      actionable: true,
  ...
  private analyzeEnergySensitivity(): BehavioralInsight | null {
    ...
    const avgEnergy = energyFeedback.reduce((sum, f) => sum + (f.actualOutcome.energyUsed || 0), 0) / energyFeedback.length;
    const avgSatisfaction = energyFeedback.reduce((sum, f) => sum + (f.actualOutcome.userSatisfaction || 0.5), 0) / energyFeedback.length;
    
    const sensitivity = avgSatisfaction > 0.7 && avgEnergy < 2.0 ? 'high' : 'moderate';
    ...
    return {
      ...
      actionable: true,
      recommendation: sensitivity === 'high' ? 'Prioritize energy-efficient operations' : 'Balance energy and comfort'
    };
  }
  ...
  private analyzeComfortPriorities(): BehavioralInsight | null {
    ...
    return {
      ...
      description: `Comfort priority: ${comfortPriority > 0.6 ? 'High' : 'Moderate'}`,
      confidence: Math.min(0.8, manualOverrides.length / 10),
      evidence: [`${comfortOverrides.length}/${manualOverrides.length} comfort-driven overrides`],
      actionable: true,
      recommendation: comfortPriority > 0.6 ? 'Prioritize comfort over efficiency' : 'Balanced approach recommended'
    };
  }
  ...
  private groupInteractionsByContext(interactions: UserInteraction[]): Map<string, UserInteraction[]> {
  ...
  private generateContextKey(interaction: UserInteraction): string {
    ...
    const tempBucket = Math.floor((interaction.context.indoorTemp || 21) / 2) * 2; // 2°C buckets
  ...
  private generateDetailedContextKey(interaction: UserInteraction): string {
    ...
    const tempBucket = Math.floor((interaction.context.indoorTemp || 21) / 1) * 1; // 1°C buckets
    const dayType = new Date(interaction.timestamp).getDay() >= 1 && new Date(interaction.timestamp).getDay() <= 5 ? 'wd' : 'we';
    
    return `${dayType}_h${hourBucket}_t${tempBucket}`;
  }
  
  private extractPattern(contextKey: string, interactions: UserInteraction[]): UserPattern {
  ...
  private extractPreference(contextKey: string, interactions: UserInteraction[]): LearnedPreference {
    ...
    const tempVariance = temps.length > 1
      ? Math.sqrt(temps.reduce((sum, temp) => sum + Math.pow(temp - avgTemp, 2), 0) / temps.length)
      : 1.5;
  ...
  private getMostCommonAction(interactions: UserInteraction[]): UserPattern['action'] {
  ...
  private getMostCommonDayOfWeek(interactions: UserInteraction[]): number {
    ...
    return parseInt(Object.entries(dayCounts)
      .sort(([, a], [, b]) => b - a)[0][0]);
  }
  ...
  private clampWeightChange(current: number, target: number, maxChange: number): number {
    const change = Math.sign(target - current) * Math.min(Math.abs(target - current), maxChange);
    return Math.max(0, Math.min(1, current + change));
  }
  
  private normalizeWeights(): void {
    const total = this.currentWeights.comfort + this.currentWeights.energyEfficiency + this.currentWeights.cost;
    if (total > 0) {
  ...
  private mergePatterns(existing: UserPattern, newPattern: UserPattern): UserPattern {
    ...
    return {
      ...existing,
      timeOfDay: existing.timeOfDay * (1 - weight) + newPattern.timeOfDay * weight,
      frequency: existing.frequency + newPattern.frequency,
      confidence: Math.min(0.95, existing.confidence + 0.1)
    };
  }
  
  private mergePreferences(existing: LearnedPreference, newPref: LearnedPreference): LearnedPreference {
    const weight = this.config.learningRate;
    
    return {
      ...existing,
      preference: {
        targetTemp: existing.preference.targetTemp * (1 - weight) + newPref.preference.targetTemp * weight,
        tolerance: existing.preference.tolerance * (1 - weight) + newPref.preference.tolerance * weight,
        energyPriority: existing.preference.energyPriority * (1 - weight) + newPref.preference.energyPriority * weight,
        comfortPriority: existing.preference.comfortPriority * (1 - weight) + newPref.preference.comfortPriority * weight,
        responsiveness: existing.preference.responsiveness * (1 - weight) + newPref.preference.responsiveness * weight
      },
      confidence: Math.min(0.95, (existing.confidence + newPref.confidence) / 2),
  ...
  private pruneOldData(): void {
    const cutoff = new Date(Date.now() - this.config.adaptationWindowDays * 24 * 60 * 60 * 1000);
  ...
  private pruneOutdatedPatterns(): void {
    const cutoff = new Date(Date.now() - this.config.patternValidityPeriod * 24 * 60 * 60 * 1000);
  ...
  private getDefaultRecommendations(context: HVACDecisionContext): any {
    return {
      ...
      confidence: 0.3,
      reasoning: 'Using default recommendations due to insufficient learning data'
    };
  }
  ...
  private generateRecommendationReasoning(
    ...
    const avgConfidence = matchingPrefs.reduce((sum, p) => sum + p.confidence, 0) / count;
    
    return `Based on ${count} similar situation(s) with ${(avgConfidence * 100).toFixed(0)}% confidence. ` +
           `Learned preference: ${combined.preference.targetTemp.toFixed(1)}°C with ` +
  ...
  getLearningStats(): {
    ...
    return {
      ...
      learningCapability: this.userInteractions.length >= this.config.minInteractionsForPattern
    };
  }
}
...
import { TemperatureReading, EnergyUsageData, WeatherData } from '../types/ai-types.ts';
...
export class PredictiveAnalyticsEngine {
  ...
  async predictIndoorTemperature(
    ...
    try {
      ...
      if (this.temperatureHistory.length < this.config.minDataPointsForPrediction) {
      ...
      const seasonal = this.getSeasonalComponent(new Date(Date.now() + hoursAhead * 60 * 60 * 1000));
      
      // Get weather influence
      const weatherInfluence = this.calculateWeatherInfluence(currentOutdoorTemp, hoursAhead);
      
      // Combine all factors
      const lastReading = this.temperatureHistory[this.temperatureHistory.length - 1];
      ...
      const confidence = this.calculatePredictionConfidence(trend, seasonal, weatherInfluence);
      ...
      const result: PredictionResult = {
        ...
        upperBound,
        lowerBound,
        factors: ['trend_analysis', 'seasonal_pattern', 'weather_influence', 'historical_data']
      };
      
      this.logger.info('✅ [Analytics Engine] Temperature prediction completed', {
      ...
      this.logger.error('❌ [Analytics Engine] Temperature prediction failed', error);
      return this.fallbackTemperaturePrediction(hoursAhead, currentOutdoorTemp);
    }
  }
  ...
  async predictEnergyUsage(
    ...
    try {
      ...
      const similarConditions = this.findSimilarEnergyConditions(plannedHvacMode);
      const avgUsage = similarConditions.reduce((sum, data) => sum + data.usage, 0) / similarConditions.length;
      
      // Apply time-of-day factor
      const futureTime = new Date(Date.now() + hoursAhead * 60 * 60 * 1000);
      const timeOfDayFactor = this.getTimeOfDayEnergyFactor(futureTime.getHours());
      ...
      const result: PredictionResult = {
        ...
        upperBound,
        lowerBound,
        factors: ['historical_average', 'time_of_day', 'efficiency_factor', 'hvac_mode']
      };
      ...
      this.logger.error('❌ [Analytics Engine] Energy prediction failed', error);
      return this.fallbackEnergyPrediction(hoursAhead, plannedHvacMode);
    }
  }
  ...
  private async updateSeasonalPatterns(): Promise<void> {
    ...
    try {
      ...
      const dailyPattern = this.detectSeasonalPattern(this.temperatureHistory, 24);
      if (dailyPattern) {
        this.seasonalPatterns.set('daily', dailyPattern);
      }
      
      // Analyze weekly pattern (7-day cycle)
      const weeklyPattern = this.detectSeasonalPattern(this.temperatureHistory, 24 * 7);
      if (weeklyPattern) {
      ...
      this.logger.error('❌ [Analytics Engine] Failed to update seasonal patterns', error);
    }
  }
  ...
  private detectSeasonalPattern(
    ...
    const averages = buckets.map(bucket => 
      bucket.length > 0 ? bucket.reduce((sum, temp) => sum + temp, 0) / bucket.length : 0
    );
    ...
    const confidence = this.calculatePatternConfidence(buckets, averages, overallAvg);
    ...
    buckets.forEach((bucket, index) => {
      if (bucket.length > 0) {
        ...
        const variance = bucket.reduce((sum, temp) => 
          sum + Math.pow(temp - bucketAvg, 2), 0
        ) / bucket.length;
   ...
   */
  private analyzeTrend(data: TemperatureReading[], hoursBack: number): TrendAnalysis {
    const cutoffTime = new Date(Date.now() - hoursBack * 60 * 60 * 1000);
    const recentData = data.filter(reading => reading.timestamp >= cutoffTime);
    ...
    return {
      ...
      significance: Math.abs(slope) > 0.5 ? 'high' : Math.abs(slope) > 0.2 ? 'medium' : 'low'
    };
  }
   ...
   */
  private calculateWeatherInfluence(currentOutdoorTemp?: number, hoursAhead: number = 1): number {
    if (!currentOutdoorTemp || this.temperatureHistory.length === 0) {
      return 0;
    }
    
    const lastIndoorTemp = this.temperatureHistory[this.temperatureHistory.length - 1].indoorTemp;
   ...
   */
  private findSimilarEnergyConditions(mode: string, maxResults: number = 20): EnergyUsageData[] {
  ...
  private pruneHistoricalData(): void {
    const cutoffTime = new Date(Date.now() - this.config.maxHistoricalDays * 24 * 60 * 60 * 1000);
  ...
  getAnalyticsSummary(): {
    ...
    return {
      ...
      predictionCapability: this.temperatureHistory.length >= this.config.minDataPointsForPrediction
    };
  }
}
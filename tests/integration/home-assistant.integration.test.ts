...
const hasHassCredentials = !!(Deno.env.get('HASS_URL') && Deno.env.get('HASS_TOKEN'));
...
Deno.test('Home Assistant Integration', async (t) => {
  ...
  if (!hasHassCredentials) {
    await t.step('should skip integration tests - no HA credentials', () => {
      console.log('⚠️  Skipping Home Assistant integration tests - HASS_URL or HASS_TOKEN not configured');
  ...
  await t.step('should discover temperature sensors', async () => {
    ...
    try {
      ...
      assertEquals(tempSensors.length > 0, true, 'Should find at least one temperature sensor');
  ...
  await t.step('should test WebSocket connection', async () => {
    ...
    try {
      ...
      if (tempSensors.length > 0) {
        ...
        console.log(`🌡️ Test sensor reading: ${sensor.entity_id} = ${reading.state}${reading.attributes?.unit_of_measurement || ''}`);
      }
  ...
  await t.step('should test service calls', async () => {
    ...
    try {
      ...
      console.log(`📋 Available services: ${Object.keys(services).length} domains`);
      
      // Check for common HVAC-related services
      const hvacServices = ['climate', 'switch', 'input_number', 'automation'];
      const availableHvacServices = hvacServices.filter(service => services[service]);
      
      console.log(`🏠 HVAC-related services available: ${availableHvacServices.join(', ')}`);
      assertEquals(availableHvacServices.length > 0, true, 'Should have at least one HVAC service available');
  ...
  await t.step('should test entity filtering', async () => {
    ...
    try {
      ...
      const entityTypes = {
        ...
        binary_sensors: states.filter(s => s.entity_id.startsWith('binary_sensor.'))
      };
      ...
      assertEquals(states.length > 0, true, 'Should have at least some entities');
      assertEquals(entityTypes.sensors.length > 0, true, 'Should have at least some sensors');
  ...
  await t.step('should handle connection errors gracefully', async () => {
    ...
    try {
      ...
      assertEquals(false, true, 'Connection should fail with invalid credentials');
    } catch (error) {
      // Expected to fail

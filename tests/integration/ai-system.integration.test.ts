...
import { PredictiveAnalyticsEngine, AnalyticsConfig } from '../../src/ai/predictive/analytics-engine.ts';
import { AdaptiveLearningEngine, LearningConfig } from '../../src/ai/learning/adaptive-learning-engine.ts';
import { SmartScheduler, SchedulingConfig } from '../../src/ai/scheduling/smart-scheduler.ts';
import { PerformanceOptimizer, PerformanceConfig } from '../../src/ai/optimization/performance-optimizer.ts';
import { HVACDecisionContext, TemperatureReading, EnergyUsageData } from '../../src/ai/types/ai-types.ts';
...
function generateTestData() {
  ...
  const temperatureReadings: TemperatureReading[] = Array.from({ length: 24 }, (_, i) => ({
...
Deno.test('AI System Integration', async (t) => {
  ...
  await t.step('should process complete decision workflow', async () => {
    ...
    console.log(`📈 Temperature prediction: ${prediction.values[0]?.toFixed(1)}°C`);
    
    // 2. Optimization
    const optimization = await system.optimizer.optimize(context);
    assertExists(optimization);
    console.log(`⚡ Optimization score: ${optimization.overallScore.toFixed(2)}`);
  ...
  await t.step('should handle data flow between components', async () => {
    ...
    const tempPrediction = await system.analytics.predictTemperature(context, 1);
    assertExists(tempPrediction);
    assertEquals(tempPrediction.values.length >= 1, true);
    
    const energyPrediction = await system.analytics.predictEnergyUsage(context, 1);
  ...
  await t.step('should handle performance monitoring', async () => {
    ...
    console.log(`✅ Performance monitoring completed (${totalTime.toFixed(2)}ms total)`);
  });
  ...
  if (hasApiKey) {
    ...
    await t.step('should skip AI pipeline test - no OpenAI API key', () => {
      console.log('⚠️  Skipping full AI pipeline test - OPENAI_API_KEY not configured');

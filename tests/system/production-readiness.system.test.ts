...
import { PerformanceOptimizer, PerformanceConfig } from '../../src/ai/optimization/performance-optimizer.ts';
import { ProductionValidator, ProductionConfig } from '../../src/core/production-validator.ts';
...
async function testPerformanceOptimizer(): Promise<boolean> {
  ...
  try {
    ...
    try {
      ...
      console.log(`  Current metrics recorded: ${currentMetrics ? '✅' : '❌'}`);
      
      if (currentMetrics) {
        console.log(`    Memory usage: ${currentMetrics.memoryUsage}MB`);
        console.log(`    CPU usage: ${currentMetrics.cpuUsage}%`);
        console.log(`    Response time: ${currentMetrics.averageResponseTime}ms`);
    ...
    try {
      ...
      const testPrediction = { value: 21.5, confidence: 0.7, factors: ['weather', 'time'] };
      optimizer.cachePrediction('temp_prediction_1h', testPrediction, 300);
      
      const cachedPrediction = optimizer.getCachedPrediction('temp_prediction_1h');
      ...
      console.log(`    Hit rate: ${(cacheStats.overall.hitRate * 100).toFixed(1)}%`);
      console.log(`    Memory usage: ${cacheStats.overall.memoryUsage.toFixed(1)}MB`);
      
      if (cachedDecision && cachedPrediction && cacheStats.overall.totalSize > 0) {
    ...
    try {
      ...
      console.log(`  All tasks completed: ${results.every(r => r.result.includes('completed')) ? '✅' : '❌'}`);
    ...
    try {
      ...
      console.log(`  Performance summary generated: ${performanceSummary ? '✅' : '❌'}`);
      console.log(`  Current metrics available: ${performanceSummary.current ? '✅' : '❌'}`);
      console.log(`  Recommendations provided: ${performanceSummary.recommendations.length}`);
      console.log(`  Optimization history: ${optimizationHistory.length} entries`);
...
async function testProductionValidator(): Promise<boolean> {
  ...
  try {
    ...
    try {
      ...
      console.log(`  Production ready: ${validationResult.ready ? '✅' : '❌'}`);
      ...
      console.log(`    Critical failures: ${validationResult.summary.critical}`);
    ...
    try {
      ...
      console.log(`    Memory: ${systemHealth.resources.memory.percentage.toFixed(1)}%`);
      console.log(`    CPU: ${systemHealth.resources.cpu.usage}%`);
      console.log(`    Disk: ${systemHealth.resources.disk.percentage.toFixed(1)}%`);
      
      const onlineComponents = Object.values(systemHealth.components)
        .filter(c => c.status === 'online').length;
      
      console.log(`  Online components: ${onlineComponents}/${Object.keys(systemHealth.components).length}`);
    ...
    try {
      ...
      console.log(`  Post-deployment items: ${checklist.postDeployment.length}`);
      ...
      console.log(`  ❌ Deployment checklist generation failed: ${error.message}`);
    }
    ...
    try {
      ...
      console.log(`  Validation categories analyzed: ${Object.keys(categories).length}`);
      
      for (const [category, stats] of Object.entries(categories)) {
        const successRate = (stats.pass / stats.total) * 100;
        console.log(`    ${category}: ${stats.pass}/${stats.total} passed (${successRate.toFixed(0)}%)`);
      }
      ...
      console.log(`  ❌ Validation categories analysis failed: ${error.message}`);
    }
...
if (import.meta.main) {
  ...
  const overallSuccess = performanceSuccess && validationSuccess && integrationSuccess;
  
  console.log('\n🏆 Overall Test Results\n');
  
  console.log('Test Summary:');
  console.log(`- Performance Optimizer: ${performanceSuccess ? '✅ PASSED' : '❌ FAILED'}`);
  console.log(`- Production Validator: ${validationSuccess ? '✅ PASSED' : '❌ FAILED'}`);
  console.log(`- Integrated System: ${integrationSuccess ? '✅ PASSED' : '❌ FAILED'}`);

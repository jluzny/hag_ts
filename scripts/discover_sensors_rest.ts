...
/**
 * Discover available temperature sensors using REST API
 */

import { ConfigLoader } from '../src/config/loader.ts';

async function discoverSensorsREST() {
  try {
    console.log('🔍 Discovering temperature sensors via REST API...\n');
    
    // Load config
    const config = await ConfigLoader.loadSettings('config/hvac_config_test.yaml');
    ...
    const indoorCandidates = tempSensors.filter((s: any) => 
    ...
    const outdoorCandidates = tempSensors.filter((s: any) => 
    ...
    if (indoorCandidates.length > 0) {
      console.log(`  tempSensor: "${indoorCandidates[0].entity_id}"  # ${indoorCandidates[0].attributes.friendly_name}`);
      if (indoorCandidates.length > 1) {
        console.log('  # Other indoor options:');
        indoorCandidates.slice(1).forEach((s: any) => 
          console.log(`  # tempSensor: "${s.entity_id}"  # ${s.attributes.friendly_name}`)
        );
      }
    }
    
    if (outdoorCandidates.length > 0) {
      console.log(`  outdoorSensor: "${outdoorCandidates[0].entity_id}"  # ${outdoorCandidates[0].attributes.friendly_name}`);
      if (outdoorCandidates.length > 1) {
        console.log('  # Other outdoor options:');
        outdoorCandidates.slice(1).forEach((s: any) => 
          console.log(`  # outdoorSensor: "${s.entity_id}"  # ${s.attributes.friendly_name}`)
        );
      }
    }
    
    if (indoorCandidates.length === 0 && outdoorCandidates.length === 0) {
      console.log('⚠️ No obvious indoor/outdoor candidates found. You may need to manually select from the list above.');
    }

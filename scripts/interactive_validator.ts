...
import { ValidationHelper, HealthChecker, ConfigValidator } from './validation_helpers.ts';
...
class InteractiveValidator {
  ...
  async run() {
    ...
    if (await this.validator.promptForValidation('Check system requirements?')) {
    ...
    if (await this.validator.promptForValidation('Check network connectivity?')) {
      this.validator.logStep('Checking network connectivity');
      const networkResults = await this.healthChecker.checkNetworkConnectivity();
      this.validator.displayResults('Network Connectivity', networkResults);
      await this.validator.waitForContinue();
    }
    
    // Configuration validation
    if (await this.validator.promptForValidation('Validate configuration files?')) {
      ...
      for (const configPath of configPaths) {
        try {
          const configResults = await this.configValidator.validateConfigFile(configPath);
          this.validator.displayResults(`Configuration: ${configPath}`, configResults);
        } catch (error) {
    ...
    if (await this.validator.promptForValidation('Test Home Assistant connectivity?')) {
      console.log('🏠 Testing Home Assistant connectivity...');
      console.log('Note: This requires HASS_URL and HASS_TOKEN environment variables');
    ...
    // AI components test
    if (await this.validator.promptForValidation('Test AI components availability?')) {
      ...
      if (openaiKey) {
        try {
          ...
          aiResults.push({
            ...
            details: openaiKey.startsWith('sk-') ? 'Valid format' : 'Invalid format'
          });
        } catch (error) {
          aiResults.push({
        ...
        if (binaryStat.isFile) {
          ...
          try {
            ...
            binaryResults.push({
              ...
              status: code === 0,
              details: code === 0 ? `Version: ${output.trim()}` : 'Execution failed'
            });
          } catch (error) {
            binaryResults.push({
              name: 'Binary Execution',
    ...
    console.log('Use the test scripts in tests/ directory for comprehensive automated testing.');
  }
}

...
if (!entityId) {
  ...
  console.log(
    "   bun run scripts/simulate_sensor_change.ts sensor.temperature 23.5",
  );
  console.log(
    "   bun run scripts/simulate_sensor_change.ts binary_sensor.door_contact on",
  );
  console.log(
    "   bun run scripts/simulate_sensor_change.ts switch.living_room_light off",
  );
  process.exit(1);
}
...
try {
  ...
  if (stateMachine) {
    ...
    if (context.indoorTemp)
      console.log(`   Indoor temp: ${context.indoorTemp}°C`);
    if (context.outdoorTemp)
      console.log(`   Outdoor temp: ${context.outdoorTemp}°C`);
  } else {
